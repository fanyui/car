	...
	<script type="text/javascript">
		$(document).ready(function(e){
			$('#newModal').on('show.bs.modal', function (event) {
			  ...
			  var modal = $(this)
			  modal.find('.modal-title').text('Car Details Assign Worker')
		                    ...
		                    @isset($unassignedCars)
		                        @foreach ($unassignedCars as $unassignedCar)
		                        	<tr>
		                                ...
		                                <td><button id="btn-new" type="button" class="btn btn-primary" data-toggle="modal" data-target="#newModal" data-mark="{{$unassignedCar->mark}}"  data-owner="{{$unassignedCar->owner}}" data-plate="{{$unassignedCar->plate_number}}" data-id="{{$unassignedCar->id}}">View</button>
		                                </td>
		                            </tr>
		                        @endforeach
		                    	@else 
        ...
        <!-- modal for new vehicule  starts -->
			...
			<div class="modal fade" id="fixingModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
			  <div class="modal-dialog" role="document">
			    <div class="modal-content">
			      <div class="modal-header">
			        <h5 class="modal-title" id="exampleModalLabel">Car Detauls</h5>
			        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
			          <span aria-hidden="true">&times;</span>
			        </button>
			      </div>
